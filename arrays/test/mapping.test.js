...
import Senators from '../src/data/senators.json'
...
describe(`Mapping`, () => {
  ...
  it(`should map out only the required properties`, () => {
    ...
    expect(results[0].party).to.equal(Senators[0].party)
    expect(results[0].gender).to.equal(Senators[0].person.gender)
    expect(results[0].firstName).to.equal(Senators[0].person.firstname)
    expect(results[0].lastName).to.equal(Senators[0].person.lastname)
  })
